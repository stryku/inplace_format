...
int main()
{
  ...
  static_assert(infmt::details::calc_size("{float}") == 48u);
  static_assert(infmt::details::calc_size("{double}") == 327u);
  static_assert(infmt::details::calc_size("{long double}") == 4954u);
  ...
  // Test long int
  const auto long_int_tester = [](auto val) {
    using value_t = decltype(val);

    if constexpr (sizeof(value_t) == 4u) {
      static_assert(type_dependent_calc_size<value_t>("{long}") == 11);
      static_assert(type_dependent_calc_size<value_t>("{long int}") == 11);
      static_assert(type_dependent_calc_size<value_t>("{signed long}") == 11);
      static_assert(type_dependent_calc_size<value_t>("{signed long int}") ==
                    11);
    } else if constexpr (sizeof(value_t) == 8u) {
      static_assert(type_dependent_calc_size<value_t>("{long}") == 20);
      static_assert(type_dependent_calc_size<value_t>("{long int}") == 20);
      static_assert(type_dependent_calc_size<value_t>("{signed long}") == 20);
      static_assert(type_dependent_calc_size<value_t>("{signed long int}") ==
                    20);
    } else {
      static_assert(dependent_false<decltype(val)>::value,
  ...
  long_int_tester((long int){});

  // Test long unsigned
  const auto unsigned_long_tester = [](auto val) {
    using value_t = decltype(val);

    if constexpr (sizeof(value_t) == 4u) {
      static_assert(type_dependent_calc_size<value_t>("{unsigned long}") ==
                    10);
      static_assert(type_dependent_calc_size<value_t>("{unsigned long int}") ==
                    10);
    } else if constexpr (sizeof(value_t) == 8u) {
      static_assert(type_dependent_calc_size<value_t>("{unsigned long}") ==
                    20);
  ...
  const auto unsigned_long_long_tester = [](auto val) {
    using value_t = decltype(val);

    if constexpr (sizeof(value_t) == 8u) {
      static_assert(
        type_dependent_calc_size<value_t>("{unsigned long long}") == 20);
      static_assert(
        type_dependent_calc_size<value_t>("{unsigned long long int}") == 20);
    } else {
      static_assert(dependent_false<value_t>::value,
                    "Architecture not supported");
    }
  ...
  unsigned_long_long_tester((unsigned long long int){});
